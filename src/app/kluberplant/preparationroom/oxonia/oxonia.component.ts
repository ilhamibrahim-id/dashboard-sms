import { Component, OnInit } from '@angular/core';
import { NgxSpinnerService } from 'ngx-spinner';
import { CountService } from 'src/app/services/count.service';

@Component({
  selector: 'app-oxonia',
  templateUrl: './oxonia.component.html',
  styleUrls: ['./oxonia.component.css']
})
export class OxoniaComponent implements OnInit {


  public resolved: boolean = false;
  public showtable: boolean = false;
  oxonia: any;
  itemsPerPage: number = 0;
  currentPage: number = 1;
  absoluteIndex(indexOnPage: number): number {
    return this.itemsPerPage * (this.currentPage - 1) + indexOnPage;
  }
  oxoniaobj: object = {};
  oxoniaarr: any = [];
  detailarr: any = [];
  loaddata:any;
  gambar: any;
  deskripsi: any = 'Loading..';
  nameMachine: any = 'Oxonia Pumps x 4';
  constructor(private spinner: NgxSpinnerService, private service: CountService) {
    window.scrollTo(0, 0);
  }
  show() {
    this.showtable = !this.showtable;
  }
  bukapdf() {
    window.open("assets/pdf/SM OCII-2.pdf", "_blank");
  }
  detaildata(id: any) {
    this.gambar = '';
    this.detailarr = [];
    for (let i = 0; i < this.oxoniaarr.length; i++) {
      if (this.oxoniaarr[i].id == id) {
        this.gambar = "http://192.168.9.47/kluber_lubrication/files/" + this.oxoniaarr[i].image;
        this.detailarr.splice(this.detailarr.lenght, 0, this.oxoniaarr[i]);

      }
    }


  }
  async ngOnInit(): Promise<void> {
    window.scrollTo(0, 0);
    this.loaddata = new Promise(resolve => {
    this.oxonia = this.service.getOxonia().subscribe(data => {
      this.oxoniaobj = data;
      Object.values(this.oxoniaobj).forEach(data => {
        var array = Object.keys(data).map(function (key) {
          return data[key];
        });
        for (let i = 0; i < array.length; i++) {
          this.oxoniaarr.splice(this.oxoniaarr.lenght, 0, array[i]);
        }

        this.spinner.hide();
        this.resolved = true;
      })


    }
    );
  });
    this.spinner.show();
    this.loaddata = await this.loaddata;
  }

}
